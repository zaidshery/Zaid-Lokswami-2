...
export default function EPaperViewerPage() {
  ...
  const [selectedCity, setSelectedCity] = useState<EPaperCitySlug>(EPAPER_CITY_OPTIONS[0].slug);
  ...
  useEffect(() => {
    ...
    const loadList = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/epapers?${query.toString()}`);
