...
export async function POST(req: NextRequest) {
  try {
    ...
    if (payload.role.length < 2) {
      return NextResponse.json(
        { success: false, error: 'Please select a role' },
        { status: 400 }
      );
    }
    ...
    if (!isValidUrl(payload.portfolioUrl)) {
      return NextResponse.json(
        { success: false, error: 'Please provide a valid portfolio URL' },
        { status: 400 }
      );
    }
    ...
    if (process.env.MONGODB_URI) {
      try {
        await connectDB();
        await CareerApplication.create(savePayload);
    ...
    await createStoredCareerApplication(savePayload);
